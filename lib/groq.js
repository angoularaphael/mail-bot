...
function buildRequestBody({ messages, maxTokens, temperature }) {
    ...
    // gpt-oss consomme des tokens de "reasoning" avant le contenu visible.
    if (isReasoningModel()) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            const { data } = await axios.post(
                API_URL,
                buildRequestBody({ messages, maxTokens, temperature }),
                {
                    headers: {
...
async function generateReplyBody({ label, subject, body, senderName, templateBody }) {
    ...
    const siteUrl = (process.env.BOXING_CENTER_SITE_URL || 'https://boxingcenter.fr/').replace(/\/?$/, '/');
    ...
    const { content } = await chatCompletion([
        {
            ...
            content: [
                ...
                'Mentionne le site web du club quand cela aide le client à consulter les informations, sans forcer si ce serait redondant.',
