...
function emailKey(emailData) {
    return emailData.messageId || emailData.message_id || `imap-uid-${emailData.uid}`;
}

/**
 * Vérifie si cet email a déjà reçu une réponse ou une transmission.
 * Persiste via Supabase / JSON — survit aux redémarrages.
 *
 * @param {string} messageId
 * @returns {Promise<{ handled: boolean, reason?: string }>}
 */
async function isAlreadyHandled(messageId) {
    ...
    if (sb) {
        try {
            const { data, error } = await sb
                .from(TABLE)
                .select('message_id,auto_replied,forwarded_to,processed,error')
                .eq('message_id', messageId)
                .maybeSingle();

            if (!error && data) {
                if (data.auto_replied) {
                    return { handled: true, reason: 'réponse déjà envoyée' };
                }
                if (data.forwarded_to) {
                    return { handled: true, reason: 'déjà transmis' };
                }
                if (data.processed && String(data.error || '').startsWith('skipped:')) {
                    return { handled: true, reason: 'déjà ignoré' };
                }
            }
        } catch (err) {
            console.warn(`[TRACKER] isAlreadyHandled: ${err.message}`);
        }
    }

    const local = readLocal().find((r) => r.message_id === messageId);
    if (local) {
        if (local.auto_replied) return { handled: true, reason: 'réponse déjà envoyée' };
        if (local.forwarded_to) return { handled: true, reason: 'déjà transmis' };
        if (local.processed && String(local.error || '').startsWith('skipped:')) {
            return { handled: true, reason: 'déjà ignoré' };
        }
    }
